...
- **Accessibilité — cibles tactiles** : toute zone cliquable (`Pressable`, `TouchableOpacity`, bouton, icône interactive) doit mesurer au moins **48 × 48 dp** de surface cliquable (recommandation Material Design / Android), au besoin via `hitSlop` ou du padding sans réduire la zone visible. Ne pas confondre avec le minimum web WCAG 2.5.8 (24 px) — voir section Accessibilité de `docs/architectureAdr.md`.
