...
- **DOCS-04** — le template des six piliers de `docs/architecture.md`, créé vide par DOCS-02, est rempli : pour chaque pilier, l'exigence chiffrée, le mécanisme qui y répond avec le fichier ou l'ADR qui le porte, et ce qui est délibérément écarté. Deux piliers reçoivent une réponse négative assumée plutôt qu'une case cochée — les coûts, faute de budget fixé au CDC et de validateur désigné, et la sobriété, aucun référentiel d'écoconception ne s'appliquant au projet. Le pilier « excellence opérationnelle » consigne l'écart le plus net : le CDC exige la résolution d'un incident critique en 4 h alors qu'aucune supervision n'est définie, la détection reposant à ce jour sur un signalement utilisateur — un délai qu'on ne peut donc pas vérifier. Remplir ces trois cases avec les mécanismes déjà en place aurait produit un document vert et faux ; le template pose lui-même qu'un pilier écarté avec son motif est traité, et qu'un pilier vide ne l'est pas
- **DOCS-04** — `CLAUDE.md` n'est plus une copie d'`AGENTS.md` : il l'importe par `@AGENTS.md` et se réduit à un point d'entrée. Les deux fichiers étaient dupliqués à 90 % et avaient divergé sur le point le plus sensible — `CLAUDE.md` imposait un worktree par feature, `AGENTS.md` décrivait un `git checkout -b` dans le clone principal. Selon le fichier lu, un assistant ne recevait pas la même consigne, et la version d'`AGENTS.md` faisait travailler plusieurs sessions dans le même répertoire. Le workflow worktree est retenu comme version unique. Les trois éléments qui n'existaient que dans `CLAUDE.md` sont repris dans `AGENTS.md` avant réduction : la règle d'accessibilité des cibles tactiles à 48 × 48 dp, le workflow worktree complet avec sa clôture et la suppression du worktree après merge, et les commandes `dev:backend` et `db:migrate`

### Corrigé

- **DEVOPS-15** — le suivi des features sortait en succès quand il ne trouvait rien. `update-feature-status` extrait l'identifiant du nom de branche puis cherche `### <ID>` dans `features.md` ; identifiant illisible ou section absente, il affichait « rien à faire » et rendait la main en vert. Le message dormait dans les logs d'un job réussi. Trois lots de `DOCS-04` et la PR #109 de `SEC-05` ont traversé le suivi sans y laisser de trace, et `DEVENV-01`, `DOCKER-01` et `API-03` sont dans le même angle mort. Les deux sorties deviennent des échecs annotés `::error::`. Comme ce workflow tourne sur `pull_request: closed`, donc après le merge, il ne peut qu'alerter : le blocage est porté par un nouveau job `Entrée de suivi` dans `ci.yml`, exécuté sur les PR et exigé avant merge, qui refuse une branche `feature/` ou `fix/` sans identifiant lisible ou sans entrée correspondante. Les branches hors convention passent sans contrôle — elles ne prétendent rien suivre, et les faire échouer apprendrait à contourner le job. `feature-in-progress` reçoit le même traitement : c'est le seul des trois workflows à lire `features.md` sur `dev`, son échec signale donc que l'entrée n'y est pas et que le statut `[~]` ne sera pas posé. Il se déclenche à la création de branche, sans PR à bloquer — l'alerte tombe au démarrage, quand la corriger coûte une entrée à écrire plutôt qu'un suivi à reconstituer. Les trois disent désormais trois choses distinctes : l'entrée est-elle sur `dev`, le travail est-il spécifié, le statut a-t-il pu être posé
...
- **DEVOPS-16** : skill `/feature-new`, qui exécute le chemin spec-first de bout en bout. Il rédige l'entrée `### XXX-00`, l'amène sur `dev` par une branche `chore/` et une pull request, attend la CI, merge, nettoie, puis crée le worktree de travail et le publie. Les cinq gestes qui séparaient une idée de feature de sa première ligne de code deviennent une commande. Il ne supprime pas la double pull request, seulement son coût humain : spécifier et réaliser restent deux actes distincts, et c'est la trace du premier qui manquait. Il s'arrête et rend la main si la CI de la pull request de spécification échoue, sans jamais forcer un merge ni contourner un check. `AGENTS.md` décrit désormais le démarrage en deux temps et la raison de l'ordre, sans nommer le skill, conformément à l'ADR 0024

- **DEVOPS-16** : entrée `### DEVOPS-16` dans `.ai/features.md`, spécifiant un skill qui automatise le chemin spec-first introduit par DEVOPS-15. L'ordre à respecter n'est pas ajustable : `feature-in-progress` lit `features.md` sur `dev` à la création de branche, et `dev` n'accepte que des pull requests, donc l'entrée ne peut y arriver que par une pull request préalable. Cinq gestes en découlent avant la première ligne de code. La mesure écarte l'hypothèse d'un coût CI : une pull request de spécification prend 65 secondes, les jobs tournant en parallèle. Ce sont les gestes qui pèsent, et aucun ne demande de jugement une fois l'entrée rédigée

- **DOCS-05** : entrée `### DOCS-05` dans `.ai/features.md`, spécifiant les trois ADR à écrire pour des décisions déjà prises et appliquées (source unique des instructions d'agent, emplacement du contrôle de suivi, protection de `dev` par ruleset avec exemption). Cette entrée arrive sur `dev` par une branche `chore/`, avant la branche de travail : c'est la seule façon pour `feature-in-progress` de trouver la section au moment de la création de branche et de poser `[~]`. Une branche `chore/` n'est pas jugée par le job `Entrée de suivi` et ne déclenche pas `feature-in-progress`, ce qui en fait le véhicule de la spécification elle-même

- **DOCS-04** — entrée `### DOCS-04` dans `.ai/features.md`, qui n'existait pas alors que trois lots de travail ont déjà été livrés sous cet identifiant. Le workflow `update-feature-status` extrait l'ID depuis le nom de branche, cherche un `### <ID>` dans `features.md`, ne le trouve pas et sort en succès — le `|| true` posé sur le `grep` pour éviter un échec transforme l'identifiant manquant en silence. Une feature pouvait donc naître, être développée et être mergée sans jamais apparaître dans le suivi, et rien ne le signalait. L'entrée porte le périmètre réel en trois items cochés, et renvoie hors périmètre les deux décisions que la documentation ne peut pas prendre à la place du projet : la supervision et le budget
