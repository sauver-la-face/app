...
- **WEB-04** — export du dossier patient depuis le dashboard. Le backend générait déjà les trois formats (EXPORT-01) sans que rien ne permette de les déclencher : le PDF du dossier et le JSON de portabilité sont proposés sur la fiche patient, le CSV anonymisé sur le répertoire. Le téléchargement passe par `fetch` et un blob, pas par un lien : la session médecin est un cookie, et une navigation vers l'origine de l'API ne l'emporte pas de façon fiable. Le nom de fichier suit `Content-Disposition` quand CORS l'expose, sinon un nom de repli
