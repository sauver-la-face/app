...
- **DOCS-05** : trois ADR pour des décisions structurantes prises et appliquées sans être consignées. `0024` établit `AGENTS.md` comme source unique des instructions d'agent, `CLAUDE.md` se réduisant à un import, après que les deux fichiers se soient contredits sur le workflow Git. `0025` place le contrôle du suivi de features dans la CI de pull request plutôt que dans les workflows de statut, ces derniers s'exécutant hors du cycle d'une pull request ouverte et ne pouvant qu'alerter. `0026` remplace la protection classique de `dev` par un ruleset à zéro approbation et huit checks, avec l'application `sauver-la-face-ci` en acteur de contournement, et documente pourquoi cette exemption est nécessaire : les workflows de statut poussent directement sur `dev`, ce qu'une branche protégée par des checks rejette. Chacun porte ses alternatives écartées, seule section qui garde de la valeur une fois la décision oubliée
