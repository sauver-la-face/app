...
- **DEVOPS-16** : entrée `### DEVOPS-16` dans `.ai/features.md`, spécifiant la réduction du coût de la CI sur les pull requests sans code. La pull request de spécification introduite par DEVOPS-15 ne touche que deux fichiers texte et déclenche pourtant les huit checks requis, dont les tests d'intégration et la construction des images Docker. Le statut `[~]` est conservé : c'est le mécanisme qui le porte qu'on rend supportable, pas le suivi qu'on abandonne
