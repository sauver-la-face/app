...
Créé le 2026-08-31 · Dernière revue : 2026-09-02
...
| A07 | Authentication Failures | partiel | `shared/middleware/patientAuthMiddleware.ts` relit le code porteur à chaque requête (401 `SESSION_REVOKED`) · `DELETE /patients/{id}/session` coupe la session · [ADR 0023](../adr/0023-couper-une-session-patient-en-revoquant-le-code-porteur.md) · blocage 15 min après 3 échecs par IP sur `/api/auth/sign-in/email` (`auth/presentation/authRouter.ts`) et sur `/auth/patient/validate` (`shared/middleware/rateLimiter.ts`) · session médecin 2 h d'inactivité en fenêtre glissante (`auth/infrastructure/authConfig.ts`) · code patient invalide, expiré, révoqué ou supprimé rejeté au niveau du domaine, chaque cas ayant son code d'erreur (`auth/application/authUsecase.ts`). **Reste :** le second facteur TOTP est disponible — plugin `twoFactor` activé dans `authConfig.ts`, donc l'enrôlement fonctionne — mais **aucune route ne l'exige** : `requirePhysicianAuth` se contente d'une session valide, un compte médecin sans MFA accède à toutes les données de santé. C'est l'écart entre AUTH-02 tel qu'écrit dans `.ai/features.md` et le code publié · limitation de tentatives en mémoire de processus, voir les notes |
...
- **MFA non exigé.** Le plugin `twoFactor` est activé, mais aucune route ne
  vérifie l'enrôlement : `requirePhysicianAuth` ne regarde que la validité de la
  session. Un compte médecin sans second facteur accède à l'intégralité des
  données de santé, alors qu'AUTH-02 l'annonce obligatoire. Bloquant.
