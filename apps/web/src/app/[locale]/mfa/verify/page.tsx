...
export default async function MfaVerifyPage(props: { params: Promise<{ locale: Locale }> }) {
      ...
      <div className="my-auto mx-auto w-full max-w-[440px]">
        <div className="rounded-[20px] border border-black/10 bg-white shadow-[0px_4px_4px_rgba(0,0,0,0.25)]">
          <div className="px-8 pb-3 pt-4">
            <h1 className="text-xl font-normal text-gray-900">{mfa.verifyPageTitle}</h1>
            <p className="mt-0.5 text-xs text-gray-500">{mfa.pageEyebrow}</p>
          </div>
          <div className="border-t border-black/20" />
          <div className="px-8 py-5">
            <MfaVerify locale={params.locale} dictionary={dictionary} />
          </div>
        </div>
